...
export const ManagerPromotionTable = ({ promotions, selectedIds, onSelectRow, onSelectAll, onToggleActive, onDelete }) => {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden relative">
      <div className="overflow-x-auto min-h-[300px]">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-blue-600/5 border-b border-gray-100 text-xs font-bold text-blue-800 uppercase tracking-wider">
              <th className="py-4 px-6 w-12 text-center">
                <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500/20 accent-blue-600 cursor-pointer"
                  checked={promotions.length > 0 && selectedIds.length === promotions.length}
                  onChange={handleToggleSelectAll} />
              </th>
              <th className="py-4 px-4">Mã giảm giá hệ thống</th>
            ...
            {promotions.length === 0 ? (
              <tr>
                <td colSpan="8" className="text-center py-16 text-gray-400 font-medium">
                  Không tìm thấy mã giảm giá hệ thống nào phù hợp.
                </td>
              </tr>
            ) : (
              promotions.map((p) => {
                ...
                return (
                  <tr key={p.id} className={`hover:bg-gray-50/40 transition-colors duration-200 ${isChecked ? 'bg-blue-50/40' : ''}`}>
                    <td className="py-4 px-6 text-center">
                      <input type="checkbox" className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500/20 accent-blue-600 cursor-pointer"
                        checked={isChecked} onChange={() => onSelectRow(p.id)} />
                    </td>
                    <td className="py-4 px-4">
                      <div className="flex flex-col">
                        <span className="font-extrabold text-gray-900 font-mono tracking-wider">{p.name}</span>
                        {p.description && <span className="text-[11px] text-gray-400 mt-0.5 line-clamp-1">{p.description}</span>}
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-black">
                        {p.discount_value}<FiPercent size={12} />
                      </span>
                    </td>
                    <td className="py-4 px-4 text-gray-600">{formatPrice(p.min_order_value)}</td>
                    <td className="py-4 px-4 text-gray-600">{p.max_discount_amount ? formatPrice(p.max_discount_amount) : 'Không giới hạn'}</td>
                    <td className="py-4 px-4 text-center">
                      <div className="flex flex-col items-center gap-0.5">
                        <span className="text-[11px] text-gray-500 flex items-center gap-1"><FiCalendar size={10} /> {formatDate(p.start_date)}</span>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Link to={`/manager/promotions/detail/${p.id}`}
                          ...
                          <TooltipContent className="rounded-lg bg-gray-800 text-white text-xs border-none font-semibold">Xem chi tiết</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Link to={`/manager/promotions/edit/${p.id}`}
                              className={`inline-flex p-2.5 rounded-xl cursor-pointer active:scale-90 transition-all duration-200 ${
                                expired ? 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed pointer-events-none'
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="rounded-lg bg-red-600 text-white text-xs border-none font-semibold">Xóa mã giảm giá</TooltipContent>
                        </Tooltip>
                      </div>
                    </td>
                  </tr>
